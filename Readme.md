...
- [0. プログラム概要](#0-プログラム概要)
  - [0.1 言葉の定義](#01-言葉の定義)
  - [0.2 関連リンク](#02-関連リンク)
- [1. SLAM-kataoka](#1-slam-kataoka)
  - [1.1 ディレクトリ構造](#11-ディレクトリ構造)
  - [1.2 点群の前処理](#12-点群の前処理)
    - [1.2.1 点群データの場所](#121-点群データの場所)
    - [1.2.2 点群データの生成](#122-点群データの生成)
      - [1.2.2.1 .csvから.pcd(途中1)を生成](#1221-csvからpcd途中1を生成)
      - [1.2.2.2 .pcd(途中1)から.pcd(途中2)を生成](#1222-pcd途中1からpcd途中2を生成)
      - [1.2.2.3 .pcd(途中2)から.pcd(完成)を生成](#1223-pcd途中2からpcd完成を生成)
  - [1.3 大域的位置合わせ](#13-大域的位置合わせ)
    - [1.3.1 設定パラメータ](#131-設定パラメータ)
    - [1.3.2 出力形式](#132-出力形式)
      - [1.3.2.1 位置合わせ結果：変位情報](#1321-位置合わせ結果変位情報)
      - [1.3.2.2 位置合わせ結果：点群データ](#1322-位置合わせ結果点群データ)
    - [1.3.3 位置合わせするフレームのスキップ](#133-位置合わせするフレームのスキップ)
    - [1.3.4 位置合わせするフレームの組み合わせのスキップ](#134-位置合わせするフレームの組み合わせのスキップ)
    - [1.3.5 位置合わせ 実行準備](#135-位置合わせ-実行準備)
    - [1.3.6 位置合わせ 実行手順](#136-位置合わせ-実行手順)
    - [1.3.7 位置合わせ結果比較 実行準備](#137-位置合わせ結果比較-実行準備)
    - [1.3.8 位置合わせ結果比較 実行手順](#138-位置合わせ結果比較-実行手順)
  - [1.4 局所的位置合わせ](#14-局所的位置合わせ)
    - [1.4.1 設定パラメータ](#141-設定パラメータ)
    - [1.4.2 出力形式](#142-出力形式)
      - [1.4.2.1 位置合わせ結果：変位情報](#1421-位置合わせ結果変位情報)
      - [1.4.2.2 位置合わせ結果：点群データ](#1422-位置合わせ結果点群データ)
    - [1.4.3 位置合わせ 実行準備](#143-位置合わせ-実行準備)
    - [1.4.4 位置合わせ 実行手順](#144-位置合わせ-実行手順)
    - [1.4.5 位置合わせ結果比較 実行準備](#145-位置合わせ結果比較-実行準備)
    - [1.4.6 位置合わせ結果比較 実行手順](#146-位置合わせ結果比較-実行手順)
- [2. ポーズ調整](#2-ポーズ調整)
  - [2.1 ディレクトリ構造](#21-ディレクトリ構造)
  - [2.2 設定パラメータ](#22-設定パラメータ)
  - [2.3 出力形式](#23-出力形式)
    - [2.3.1 ポーズ調整：ポーズ調整前の軌跡](#231-ポーズ調整ポーズ調整前の軌跡)
    - [2.3.2 ポーズ調整：変位情報](#232-ポーズ調整変位情報)
    - [2.3.3 ポーズ調整：ポーズ調整後の軌跡](#233-ポーズ調整ポーズ調整後の軌跡)
    - [2.3.4 ポーズ調整：フレーム全体を通した評価値](#234-ポーズ調整フレーム全体を通した評価値)
  - [2.4 位置合わせ 実行準備](#24-位置合わせ-実行準備)
  - [2.5 位置合わせ 実行手順](#25-位置合わせ-実行手順)
  - [2.6 位置合わせ結果比較 実行準備](#26-位置合わせ結果比較-実行準備)
  - [2.7 位置合わせ結果比較 実行手順](#27-位置合わせ結果比較-実行手順)
