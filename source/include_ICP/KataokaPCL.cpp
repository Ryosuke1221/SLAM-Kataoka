...
Eigen::Affine3f calcAffine3fFromMatrix4d(Eigen::Matrix4d input_Mat) {
	Eigen::Affine3f Trans_Affine = Eigen::Affine3f::Identity();
	Eigen::Vector6d Trans_Vec = Eigen::Vector6d::Identity();
	Trans_Vec = calcXYZRPYFromPositoinMatrix(input_Mat);
...
void CKataokaPCL::transformPointCloud(pcl::PointCloud<pcl::PointXYZRGB> &input_arg,
	...
	trans_ = calcAffine3fFromMatrix4d(transformation_arg.cast<double>());
...
vector<int> CKataokaPCL::getCharacter(Eigen::Matrix4d TRUESensorPos, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_) {
	...
	Trans_PC = calcAffine3fFromMatrix4d(TRUESensorPos);
